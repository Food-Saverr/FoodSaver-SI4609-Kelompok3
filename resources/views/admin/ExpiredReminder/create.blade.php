...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-orange-600">Buat Notifikasi Expired Reminder</h1>
        ...
        <form action="{{ route('admin.expired-reminders.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="makanan_id" class="block text-sm font-medium text-gray-700 mb-1">
                        Pilih Makanan
                    </label>
                    <select name="makanan_id" id="makanan_id" 
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 @error('makanan_id') border-red-500 @enderror">
                        <option value="">-- Pilih Makanan --</option>
                        @foreach($makanans as $makanan)
                            <option value="{{ $makanan->ID_Makanan }}" {{ old('makanan_id') == $makanan->ID_Makanan ? 'selected' : '' }}>
                                {{ $makanan->Nama_Makanan }} - {{ $makanan->donatur->nama_user }}
                            </option>
                        @endforeach
                    </select>
                    @error('makanan_id')
                ...
                <div>
                    ...
                    <input type="datetime-local" name="send_at" id="send_at"
                           ...
                           value="{{ old('send_at', now()->addHours(1)->format('Y-m-d\TH:i')) }}">
                    @error('send_at')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="status" 
                        ...
                        <option value="sent" {{ old('status', 'pending') === 'sent' ? 'selected' : '' }}>Sent</option>
                        <option value="failed" {{ old('status', 'pending') === 'failed' ? 'selected' : '' }}>Failed</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
