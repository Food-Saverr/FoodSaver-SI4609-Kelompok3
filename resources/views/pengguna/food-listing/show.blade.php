                        ...
                        <div class="md:col-span-5">
                            ...
                            <div class="relative rounded-2xl overflow-hidden bg-gray-100 shadow-md h-72 mb-5">
                                ...
                                <div class="absolute top-4 right-4">
                                    ...
                                    @endphp
                                    <span class="px-4 py-2 rounded-full text-sm font-medium shadow-md
                            ...
                            <div class="p-5 bg-orange-50 rounded-xl shadow-sm">
                                ...
                                <div class="flex items-center">
                                    <img src="{{ $makanan->donatur->Foto_Profil ? asset('storage/' . $makanan->donatur->Foto_Profil) : asset('images/user-placeholder.jpg') }}" 
                                         class="w-16 h-16 rounded-full border-2 border-orange-200" 
                                         alt="{{ $makanan->donatur->Nama_Pengguna ?? 'Donatur' }}">
                                    <div class="ml-4">
                                        <p class="font-medium text-gray-800 text-lg">{{ $makanan->donatur->Nama_Pengguna ?? 'Pengguna #' . $makanan->ID_Pengguna }}</p>
                                        <p class="text-gray-500">{{ $makanan->donatur->Role_Pengguna ?? 'Donatur' }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    @if($makananHabis || $isExpired)
                        <div class="text-center bg-red-50 rounded-xl p-6 shadow-sm mb-6">
                            ...
                            <a href="{{ route('pengguna.food-listing.index') }}" 
                               class="inline-block bg-gradient-to-r from-gray-500 to-gray-600 text-white py-2.5 px-6 rounded-xl font-semibold hover:from-gray-600 hover:to-gray-700 transition animate-scale">
                                Lihat Makanan Lain
                            </a>
                        </div>
                    @elseif(auth()->check() && auth()->user()->Role_Pengguna === 'Pengguna')
                        <form action="{{ route('pengguna.food-listing.store-request', $makanan->ID_Makanan) }}" method="POST" class="space-y-4">
                            @csrf
                            <div>
                                <label for="pesan" class="block text-gray-700 font-medium mb-2">Pesan (Opsional)</label>
                                <textarea name="Pesan" id="pesan" rows="4" 
                                          placeholder="Tambahkan pesan untuk donatur..." 
                                          class="w-full rounded-xl border border-gray-200 bg-white/90 focus:outline-none focus:border-orange-400 input-focus-effect transition-all p-3 text-gray-700"></textarea>
                            </div>
                            <button type="submit" 
                                    class="w-full bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white py-3 rounded-xl font-semibold transition animate-scale shadow-lg shadow-orange-200">
                        ...
                        <div class="text-center bg-gray-50 rounded-xl p-6 shadow-sm mb-6">
                            ...
                            @guest
                                <a href="{{ route('login.form') }}" 
                                   class="inline-block bg-gradient-to-r from-orange-500 to-orange-600 text-white py-2.5 px-6 rounded-xl font-semibold hover:from-orange-600 hover:to-orange-700 transition animate-scale">
