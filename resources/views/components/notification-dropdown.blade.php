    ...
    <div x-show="open" 
        ...
        <div class="max-h-96 overflow-y-auto">
            @forelse($notifications as $notification)
                <div class="p-4 {{ $notification->is_read ? 'bg-white' : 'bg-blue-50' }} hover:bg-gray-50 border-b last:border-b-0">
                    <div class="flex justify-between items-start">
                        ...
                        @if(!$notification->is_read)
                            <button onclick="markAsRead({{ $notification->id }}, '{{ $role }}')" class="text-xs text-blue-600 hover:text-blue-800">
                                Mark as read
                            </button>
                        @endif
                    </div>
        ...
        <div class="p-4 border-t">
            <a href="/{{ $role }}/notifications" class="block text-center text-sm text-blue-600 hover:text-blue-800">
...
document.getElementById('mark-all-read').addEventListener('click', function() {
    const role = '{{ $role }}';
    fetch(`/${role}/notifications/mark-all-read`, {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        }
